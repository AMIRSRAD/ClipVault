...
pub fn paste_item(state: State<'_, AppState>, id: String) -> Result<(), String> {
    ...
    state.storage.mark_used(&id).map_err(|error| error.to_string())
}
...
pub fn pin_item(state: State<'_, AppState>, id: String, pinned: bool) -> Result<(), String> {
    state.storage.pin(&id, pinned).map_err(|error| error.to_string())
}

#[tauri::command]
pub fn set_tags(state: State<'_, AppState>, id: String, tags: Vec<String>) -> Result<(), String> {
    state.storage.set_tags(&id, tags).map_err(|error| error.to_string())
}

#[tauri::command]
pub fn create_note(state: State<'_, AppState>, text: String, tags: Vec<String>) -> Result<ClipboardItem, String> {
    state.storage.create_note(text, tags).map_err(|error| error.to_string())
}

#[tauri::command]
pub fn update_note(state: State<'_, AppState>, id: String, text: String) -> Result<ClipboardItem, String> {
    state.storage.update_note(&id, text).map_err(|error| error.to_string())
}
...
pub fn run_ocr(state: State<'_, AppState>, id: String) -> Result<OcrResponse, String> {
    ...
    let image_png = if item.kind == crate::models::ClipboardKind::Image {
        state.storage.image_blob(&id).map_err(|error| error.to_string())?
    } else {
        None
    };
    ...
    if response.status == "ready" {
        if let Some(text) = &response.text {
            state.storage.set_ocr_text(&id, text).map_err(|error| error.to_string())?;
        }
    }
    Ok(response)
}
...
#[tauri::command]
pub fn update_settings(state: State<'_, AppState>, settings: AppSettings) -> Result<AppSettings, String> {
    state.storage.update_settings(&settings).map_err(|error| error.to_string())?;
    Ok(settings)
}

#[tauri::command]
pub fn pause_capture(state: State<'_, AppState>, duration_seconds: Option<i64>) -> Result<(), String> {
    let mut pause_until = state.pause_until.lock().map_err(|_| "pause lock poisoned".to_string())?;
...
pub fn export_backup(state: State<'_, AppState>) -> Result<String, String> {
    state.storage.export_backup().map_err(|error| error.to_string())
}

#[tauri::command]
pub fn import_backup(state: State<'_, AppState>, backup: String) -> Result<usize, String> {
    state.storage.import_backup(&backup).map_err(|error| error.to_string())
}
